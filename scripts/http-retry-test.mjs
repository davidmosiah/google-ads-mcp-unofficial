// HTTP retry middleware tests — adapted from whoop-mcp.
...
// Case 1: 503 then 200 — retried once, returns success.
{
  ...
  const retryLogs = captured.filter((m) => m.startsWith('[google_ads-mcp] retry'));
  assert.equal(retryLogs.length, 1, 'one retry log line expected');
  assert.match(retryLogs[0], /status=503/);
}

// Case 2: 429 with Retry-After.
captured.length = 0;
{
  let calls = 0;
  let observedSleep = -1;
...
// Case 3: GOOGLE_ADS_NO_RETRY=true disables.
captured.length = 0;
process.env.GOOGLE_ADS_NO_RETRY = 'true';
{
  let calls = 0;
  const fetchMock = async () => {
    calls += 1;
...
delete process.env.GOOGLE_ADS_NO_RETRY;

// Case 4: 401 is NOT retried.
captured.length = 0;
{
  let calls = 0;
...
// Case 5: max attempts exhausted returns last response.
captured.length = 0;
{
  let calls = 0;
