...
async function runHttp(): Promise<void> {
  const app = express();
  const host = process.env.GOOGLE_ADS_MCP_HOST ?? "127.0.0.1";
  const port = Number(process.env.GOOGLE_ADS_MCP_PORT ?? 3000);
  const allowedOrigin = process.env.GOOGLE_ADS_MCP_ALLOWED_ORIGIN ?? `http://${host}:${port}`;
...
if (cliResult !== undefined) {
  ...
  const transport = process.env.GOOGLE_ADS_MCP_TRANSPORT ?? (args.has("--http") ? "http" : "stdio");
