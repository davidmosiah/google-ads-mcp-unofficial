...
export const DEFAULT_PROFILE: WellnessProfileDocument = {
  schema: "delx-wellness-profile/v1",
  generated_by: "google-ads-mcp-unofficial",
  version: 1,
  profile: {
    preferred_name: "",
    language: "en",
    ...
    if (typeof value === "string") {
      if (SECRET_VALUE_PATTERNS.test(value)) {
        ...
        throw new Error(
          ...
          `Profile is for non-secret wellness context only; keep tokens in each connector's own local config (e.g. ~/.google-ads-mcp/, ~/.oura-mcp/, etc.).`,
        );
      }
      return;
    }
...
export function getOnboardingFlow(locale: WellnessLanguage = "en"): OnboardingFlow {
  return {
    ...
    privacy_note:
      ...
      "Tokens stay in each connector's own local config (e.g. ~/.google-ads-mcp/, ~/.oura-mcp/, ~/.garmin-mcp/, etc.), " +
