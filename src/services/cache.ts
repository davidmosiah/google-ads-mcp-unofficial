...
export class GoogleAdsCache {
  ...
  get(method: string, url: string, body?: string): unknown | undefined {
    const row = this.db.prepare("SELECT payload FROM api_cache WHERE cache_key = ?").get(cacheKey(method, url, body)) as { payload?: string } | undefined;
  ...
  set(method: string, url: string, payload: unknown, body?: string): void {
    this.db.prepare(`
      INSERT INTO api_cache (cache_key, method, url, body, payload, cached_at)
      VALUES (@cache_key, @method, @url, @body, @payload, @cached_at)
      ON CONFLICT(cache_key) DO UPDATE SET
        payload = excluded.payload,
        cached_at = excluded.cached_at
    `).run({
      cache_key: cacheKey(method, url, body),
      method,
      url,
      body: body ?? null,
...
function cacheKey(method: string, url: string, body?: string): string {
  return body ? `${method.toUpperCase()} ${url}\n${body}` : `${method.toUpperCase()} ${url}`;
}
