...
export type LocalGoogleAdsConfig = Partial<Record<LocalConfigKey, string>>;

export interface LocalConfigReadResult {
  path: string;
  exists: boolean;
  values: LocalGoogleAdsConfig;
  permissions?: string;
...
export function getLocalConfigPath(homeDir = homedir()): string {
  return join(homeDir, ".google-ads-mcp", "config.json");
}
...
export function readLocalConfig(homeDir = homedir()): LocalConfigReadResult {
  ...
  try {
    ...
    const values = Object.fromEntries(
      LOCAL_CONFIG_KEYS
        .map((key) => [key, typeof raw[key] === "string" && (raw[key] as string).trim() ? (raw[key] as string).trim() : undefined] as const)
        .filter(([, value]) => value)
    ) as LocalGoogleAdsConfig;
...
export function writeLocalConfig(values: LocalGoogleAdsConfig, homeDir = homedir()): string {
  const path = getLocalConfigPath(homeDir);
...
export function loadConfigSources(env: Record<string, string | undefined> = process.env, homeDir = homedir()): EffectiveConfigSources {
  ...
  const values: LocalGoogleAdsConfig = {};
  let envUsed = false;
  let localUsed = false;
