...
import type { GoogleAdsTokenSet } from "../types.js";
...
export class TokenStore {
  ...
  async read(): Promise<GoogleAdsTokenSet | null> {
    try {
      const text = await fs.readFile(this.tokenPath, "utf8");
      return JSON.parse(text) as GoogleAdsTokenSet;
    } catch (error) {
  ...
  async write(tokens: GoogleAdsTokenSet): Promise<void> {
